    ...
    <section className="contract-sheet" aria-label="담당자별 계약 성과">
      ...
      <div className="contract-sheet-overflow" tabIndex={0} role="region" aria-label="담당자별 계약 표, 좌우로 스크롤 가능">
        <table style={{ minWidth: 40 + groups.length * 400 }}>
          <caption className="sr-only">담당자별 계약 업체, 채널, 등급, 프로그램, 계약액과 합계</caption>
          ...
          <thead>
            ...
            <tr className="contract-sheet-columns">{groups.map((group) => <Fragment key={group.owner}>
              <th scope="col" className="contract-sheet-divider">업체명</th><th scope="col">채널</th><th scope="col">등급</th><th scope="col">프로그램</th><th scope="col">계약액</th>
            </Fragment>)}</tr>
          </thead>
          ...
          <tfoot><tr>
            ...
            {groups.map((group) => <Fragment key={group.owner}>
              <td colSpan={4} className="contract-sheet-divider contract-sheet-count">{group.owner} · 계약 {group.rows.length}건</td>
