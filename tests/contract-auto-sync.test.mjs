...
import { _crmParseContractSource as parse, _crmPlanContractAutoSync as plan, _crmSyncMoney as money } from '../scripts/contract-sheet-sync-core.mjs';
...
test('Apps Script replans after revision conflict instead of replaying old money patch',()=>{
  ...
  const sandbox={_crmContractSourceGroups:()=>source(),_crmReadSupabasePrimaryEnvelope_:()=>({revision:'r'+(++read),data:JSON.stringify({leads:[lead(read>1?{contractAmount:999}: {})]})}),
    _crmPlanContractAutoSync:plan,_crmIso:()=>now,_crmHash:hash,_crmSaveMutationV2:()=>{save++;return {error:'revision_conflict'};}};
...
test('Apps Script dry-run never writes and failure never claims success',()=>{
  ...
  let saves=0;
  const sandbox={_crmContractSourceGroups:()=>source(),_crmReadSupabasePrimaryEnvelope_:()=>({revision:'r',data:JSON.stringify({leads:[lead()]})}),_crmPlanContractAutoSync:plan,_crmIso:()=>now,_crmHash:hash,
